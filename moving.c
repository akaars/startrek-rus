...
void probe(void) {
	...
	if (future[FDSPROB] != 1e30) {
		...
		skip(1);
		if (REPORTS) {
			prout("Ухура- \"Предыдущий зонд все еще шлет информацию.\"");
		} else {
			prout("Спок-  \"По имеющимся данным зонд все еше не ");
			prout("   достиг пункта назначения.\"");
		}
		return;
	}
	key = scan();
	...
	if (key == IHALPHA && strcmp(citem,"armed") == 0) {
		...
		proutn("Активировать боеголовку NOVAMAX?");
		isarmed = ja();
	}
	getcd(TRUE, key);
	if (direc == -1.0) return;
	...
	prout("Мичман Чеков-  \"Зонд глубокого космоса запущен, капитан.\"");
	return;
}
...
void help(void) {
	...
	if (condit == IHDOCKED) {
		prout("Лт. Ухура-  \"Но капитан, мы уже произвели стыковку.\"");
		return;
	}
	if (damage[DRADIO] != 0) {
	...
	if (d.rembase==0) {
		prout("Лт. Ухура-  \"Капитан, Звездная база не отвечает.\"");
		return;
	}
	if (landed == 1) {
		proutn("Вы должны быть на борту ");
		crmshp();
		prout(".");
		return;
	}
	...
	quad[sectx][secty]=IHDOT;
	proutn("Звездная база в ");
	cramlc(1, quadx, quady);
	proutn(" ответы--");
	crmshp();
	prout(" дематериализована.");
	...
	for (l = 1; l <= 3; l++) {
		...
		proutn(" попытка ре-материализации ");
		crmshp();
		prouts(" . . . . . ");
		if (Rand() > probf) break;
		prout("fails.");
	...
	for (l = 1; l <= 5; l++) {
		...
		if (ix>=1 && ix<=10 && iy>=1 && iy<=10 && quad[ix][iy]==IHDOT) {
			...
			dock();
			skip(1);
			prout("Лт. Ухура-  \"Капитан, мы сделали это!\"");
			return;
		}
	}
	finish(FMATERIALIZE);
